...
- Responded to all requests provided by the reviewer
  ...
  - Added executable examples wrapped in () in all exported functions
  - Ensured changes to the user’s par() settings were reset using
    on.exit as directed
  - Ensured my functions are not modifying in the user’s home filespace.
    I am using a path provided by tempdir()
- Major changes

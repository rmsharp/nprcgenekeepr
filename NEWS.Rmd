...
- `reportMatePairs()` can follow your center's ancestry rules (issue
  #169): the new optional `ancestryRules` and `overriddenRules` arguments
  take the same rules file used for breeding groups. A pair a "block" rule
  names moves to the excluded list with the reason "ancestry rule", a pair
  a "flag" rule names stays listed and is marked, and a rule you override
  keeps its pairs listed, marked "overridden". The result also shows how
  many animals each ancestry group holds and which ones no rule reaches.
  Without rules the report is exactly as before.

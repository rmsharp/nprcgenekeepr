...
- Mate pair reports can follow your center's ancestry rules (issue #169).
  ...
  explains what the rules do there. In scripts, `reportMatePairs()` takes
  the same rules through the new optional `ancestryRules` and
  `overriddenRules` arguments; a rule you override keeps its pairs listed,
  marked "overridden". The result also shows how many animals each
  ancestry group holds and which ones no rule reaches. Without rules, or
  when the pedigree has no ancestry column, the report and the tab are
  exactly as before.

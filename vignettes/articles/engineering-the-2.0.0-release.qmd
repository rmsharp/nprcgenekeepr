...
`NPRC_RUN_E2E`, and executes the 23-file `^(app|e2e)-` tier as the
13-group partition 8e-7 added -- distinct from `R-CMD-check.yaml` and
`test-coverage.yaml`, which remain the fast, per-pull-request gate.
...
sessions -- including all four sessions that produced Sections 1-3 and the

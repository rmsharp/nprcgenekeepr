...
narrow the focal-animal selection first (the **Focal Animals** panel to the
left of the diagram; see the Colony Manager's Guide article for the full
...
mating-to-child connector between two routings: **Rectilinear
(kinship2-style)** (the default -- strict horizontal/vertical right angles
throughout: a horizontal segment between the two parents, a vertical drop
to the mating dot, a horizontal bar across the children, and a vertical
drop into each one) and **Direct** -- a single straight or lightly sloped
line all the way from parent to mating dot to child.
...
## Reading classic breeding structures {#sec-classic-structures}
...
Each is small enough (11--14 animals) to read in full on this tab, and each
contains exactly one consanguineous mating, so together they are practice
material for the two cues introduced above: the **curved dashed line**
joining an animal's repeated appearances, and the **vermillion mate-line
pair** flagging a mating between blood relatives. To try one in the app,
ask R for the file's location on disk and upload it like any pedigree
file:
...
![The full-sibling example (`example_pedigree_consanguinity.csv`): CS1 appears twice, joined by the dashed line; the vermillion mate-lines mark his mating with his full sister CD1.](pedigree-diagram-img/exemplar-consanguinity-rectilinear.png){fig-alt="Pedigree diagram of a full-sibling mating: founders CGS and CGD at top; their four children in the second row, where CS1 appears twice joined by a curved dashed line; a thick vermillion mate-line joins CS1's second occurrence to his full sister CD1; their children CI1 and CI2 below each mate an unrelated animal in the bottom rows."}
...
vermillion mate-lines mark the CS1 x CD1 mating, and its children CI1 and
CI2 are the inbred animals: each has an inbreeding coefficient of
F = 0.25, the highest a mating between two non-inbred animals can
produce. Every other animal in the pedigree, including CI1's and CI2's
own offspring by unrelated mates, has F = 0.

### Parent-offspring backcross

![The backcross example (`example_pedigree_backcross.csv`): sire BP appears twice; the vermillion mate-lines mark his mating with his own daughter BR.](pedigree-diagram-img/exemplar-backcross-rectilinear.png){fig-alt="Pedigree diagram of a parent-offspring backcross: founders BP and BQ at top; BP appears a second time in the next row, joined to his first occurrence by a curved dashed line, with a thick vermillion mate-line to his daughter BR; their children BC1, BC2, and BC3 below; BC1 mates unrelated BU with children BO1 and BO2 at bottom."}

Founder BP mates BQ, then is bred back to his own daughter BR. His second
occurrence sits one row down, next to BR, with the dashed line climbing
back to his founder-row occurrence -- an animal reappearing *below* its
own first occurrence is the tell-tale of a cross-generation mating, and
here the vermillion marker confirms the mates are also blood relatives.
The vermillion
mating's children BC1--BC3 each have F = 0.25, the same as a full-sibling
mating -- the two structures concentrate ancestry equally. BC1's own
children by the unrelated male BU are back to F = 0.

### Half-sibling mating

![The half-sibling example (`example_pedigree_half_sib.csv`): shared sire HS appears twice, once per dam; the vermillion mate-lines mark the half-sibling mating HA1 x HB1.](pedigree-diagram-img/exemplar-half_sib-rectilinear.png){fig-alt="Pedigree diagram of a half-sibling mating: sire HS appears twice at top, once with dam HD1 and once with dam HD2, the two occurrences joined by a curved dashed line; in the second row a thick vermillion mate-line joins HA1 to HB1, who also appears twice; their children HC1 and HC2 below; HC1 mates unrelated HU with children HO1 and HO2 at bottom."}
...
![The first-cousin example (`example_pedigree_first_cousin.csv`): FC2 appears twice; the vermillion mate-lines mark the first-cousin mating FC1 x FC2.](pedigree-diagram-img/exemplar-first_cousin-rectilinear.png){fig-alt="Pedigree diagram of a first-cousin mating: founders FGS and FGD at top; their full-sibling children FS1 and FS2 each mate an unrelated founder in the second row; in the third row a thick vermillion mate-line joins the cousins FC1 and FC2, with FC2 appearing twice joined by a curved dashed line; their children FF1 and FF2 at bottom."}
...
![The linebreeding example (`example_pedigree_linebreeding.csv`): influential founder LK appears twice, as does his descendant LB2; the vermillion mate-lines mark the mating LB2 x LA2, which unites two lines of descent from LK.](pedigree-diagram-img/exemplar-linebreeding-rectilinear.png){fig-alt="Pedigree diagram of linebreeding: founder LK appears twice at top, once with dam LW1 and once with dam LW2, joined by a curved dashed line; his descendants through each dam form two family branches; in a lower row a thick vermillion mate-line joins LB2 to LA2, with LB2 appearing twice joined by a long curved dashed line; their children LL1, LL2, and LL3 at bottom."}
...
- The five classic-structure example pedigrees
  (`example_pedigree_*.csv` in the package's `extdata/examples` folder) --
  see "Reading classic breeding structures" above.

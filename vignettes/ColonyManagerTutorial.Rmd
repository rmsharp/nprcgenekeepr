...
It is important to make sure the minimum breeding age is appropriate for the
animals in your pedigree. The app provides two fields -- __Minimum Sire Age__
and __Minimum Dam Age__ -- each left blank by default so that the species- and
sex-specific breeding age is used automatically. Type a number in either field
to override that sex's floor; for macaques, which may reproduce as early as two
years of age, a low value such as 2 is appropriate.

The sequence below illustrates how such an age input works, with the center
image demonstrating how the hovertext explains the value. (It shows the earlier
single-field layout; the current app splits the input into the separate sire
and dam fields described above.)


```{r example-pedigree-minParentAgeSequence, eval = TRUE, fig.width = 11, fig.height = 6, echo = FALSE}
#| fig.alt: >
#|   Input of the minimum breeding age.
img <- readPNG(file.path(".", "shiny_app_use",
...
and at risk of being lost from the population. A range of 100 to 10,000
...
Genome uniqueness is an *estimate* produced by the gene-drop simulation, so
each value carries a sampling __standard error__ -- reported in the `guSE`
column beside it. That standard error shrinks roughly in proportion to one
over the square root of the number of iterations, so more iterations give a
more precise estimate. How many iterations are actually *needed*, though,
depends on the structure of your pedigree: there is no single iteration count
that is right for every colony.

The default is __1000 iterations__. On the pedigrees we have examined this
gives a sampling error below one percentage point in the genome uniqueness
values and -- more importantly -- a stable *selection order*. The order in
which animals are ranked is what actually decides which animals are chosen
for breeding, and a small `guSE` does not by itself guarantee that order has
settled. To check whether 1000 iterations is enough for a particular pedigree,
run `gvaConvergence()`: it recomputes the ranking across a range of iteration
counts and reports the smallest count at which the selection order stops
changing. See the *Gene-Drop Iteration Convergence* vignette
(`vignette("gvaConvergence", package = "nprcgenekeepr")`) for a worked example.

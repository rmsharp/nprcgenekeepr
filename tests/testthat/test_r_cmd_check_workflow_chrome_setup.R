# These tests guard .github/workflows/R-CMD-check.yaml AND its scheduled twin,
# .github/workflows/R-CMD-check-scheduled.yaml, against the intermittent
...
# chromote::find_chrome() resolves before any test runs. R-CMD-check.yaml
# adopted the same 3-step pattern (S618/S619), but R-CMD-check-scheduled.yaml
# -- a near-duplicate workflow with the identical 5-leg matrix, triggered
# weekly by cron rather than on push -- never received it, because this test
# file originally guarded only R-CMD-check.yaml by hardcoded path. That gap
# went unnoticed until R-CMD-check-scheduled's ubuntu-latest (release) leg hit
# the exact same ambient-Chrome-discovery flake live (found S629, 2026-08-24,
# run 32710819747 -- the immediate rerun of the same job passed clean,
# confirming the flake is real but intermittent, exactly like the original
# S616/S618 findings on R-CMD-check.yaml before it got the fix). These tests
# now loop over BOTH workflow files so neither can silently drop or fall
# behind the other's copy of the pattern again (the same regression class
# test_shinytest2_workflow_coverage.R guards for the sibling workflow's own
# group partition).

workflow_dir <- testthat::test_path("..", "..", ".github", "workflows")

## Both workflows run the identical 5-leg matrix (macos-latest/windows-latest/
## ubuntu-latest x3) and both depend on chromote via the same test suite, so
## both need the identical Chrome-provisioning pattern.
workflow_files <- c("R-CMD-check.yaml", "R-CMD-check-scheduled.yaml")
...
for (workflow_file in workflow_files) {
  ## Local binding per loop iteration, captured by each test_that() closure
  ## below at the time it is defined (not lazily), so each test checks its
  ## own workflow_file/workflow_path pair, not whichever the loop landed on
  ## last.
  workflow_path <- file.path(workflow_dir, workflow_file)

  test_that(sprintf(
    "%s provisions a pinned Chrome via browser-actions/setup-chrome@v2",
    workflow_file
  ), {
    ...
    expect_true(
      ...
      info = sprintf(
        "%s does not provision Chrome via browser-actions/setup-chrome@v2 (the shinytest2.yaml-proven pattern)",
        workflow_file
      )
    )
    expect_true(
  ...
  test_that(sprintf(
    "%s points chromote at the installed Chrome via CHROMOTE_CHROME",
    workflow_file
  ), {
    skip_if_not(file.exists(workflow_path),
                sprintf("%s not present in this build", workflow_file))
    ...
    expect_true(
      ...
      info = sprintf(
        "%s does not export CHROMOTE_CHROME from the setup-chrome step's chrome-path output",
        workflow_file
      )
    )
    expect_true(
  ...
  test_that(sprintf(
    "%s asserts Chrome is resolvable by chromote before check-r-package runs",
    workflow_file
  ), {
    skip_if_not(file.exists(workflow_path),
                sprintf("%s not present in this build", workflow_file))
    ...
    expect_true(
      ...
      info = sprintf(
        "%s does not assert chromote::find_chrome() resolves before tests run",
        workflow_file
      )
    )
    ...
    expect_true(
      ...
      info = sprintf(
        "%s: one or more of the 4 ordering anchors (setup-chrome / CHROMOTE_CHROME / find_chrome() / check-r-package) is missing",
        workflow_file
      )
    )
    expect_true(
      ...
      info = paste0(
        workflow_file, ": Chrome provisioning steps are not in the required order ",
  ...
  test_that(sprintf(
    "%s skips the Chrome-provisioning steps on macos-latest (S619 CDP-timeout fallback)",
    workflow_file
  ), {
    ...
    for (pattern in chrome_step_patterns) {
      ...
      expect_true(
        ...
        info = paste0(
          workflow_file, ": the step matching '", pattern, "' must carry an `if:` guard ",

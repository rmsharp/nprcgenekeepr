...
# =============================================================================
# 6b. ORIP-gate getSiteInfo() guard (BACKLOG: "Unprotected getSiteInfo() call
#     at the ORIP-tab gate") -- a present-but-malformed site-config file (e.g.
#     missing the required 'center' key) makes getParamDef() stop(), which
#     previously propagated uncaught through the unwrapped getSiteInfo() call
#     at appServer.R:347, crashing app boot. This is the same issue #50 crash
#     class loadSiteConfig() was built to prevent (see test_loadSiteConfig.R),
#     but loadSiteConfig()'s own protection only covers its own call site, not
#     this independent, unwrapped getSiteInfo() call a few lines later.
# =============================================================================

test_that("appServer boots without crashing when the site config file is malformed", {
...
test_that(paste("appServer does not mount the ORIP module when the site",

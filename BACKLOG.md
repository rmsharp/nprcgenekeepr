...
per-item reachability notes and traps. Suggested next: NEW-53.)*
- [ ] **NEW-53** — `makeSimPed` mutates the caller's ped in place; also
      `getPotentialParents.R:28` does `data.table::setDT(ped)` without `copy()`. Fix both together.
- [ ] **NEW-45** — `geneDrop` period-in-id allele mis-assignment.

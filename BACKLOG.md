...
win-builder run. Next (owner action, unchanged): decide whether to
re-trigger win-builder/R-hub (this fix should also resolve the new
NOTE), then `devtools::submit_cran()` and click the maintainer-email
confirmation link – all still owner-only per SAFEGUARDS and the
runbook’s HARD STOP.

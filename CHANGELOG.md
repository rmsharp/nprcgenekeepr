...
- **Remaining for CRAN:** Phase 4 (`R CMD check --as-cran` gate), Phase 5 (cross-platform + `cran-comments.md`; owner submits). Phase 1 static hygiene (build cruft, the `'mulatto'`→`'mulatta'` typo, `\value` docs) and Phase 2 (archival timing root cause) are still open. [[news-vs-changelog]] [[backlog-vs-changelog-placement]]

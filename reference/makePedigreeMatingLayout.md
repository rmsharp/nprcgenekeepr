...
#> Warning: makePedigreeMatingLayout(): 509 same-row edge-node collision(s) could not be fully resolved (residual after the repair-pass cap, or a curved connector whose arc no roundness step fully clears) -- rendered output may still show a straight or curved edge passing near an unrelated node.
```

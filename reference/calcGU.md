...
Dataframe `rows: id, col: gu` A single-column table of genome uniqueness
values as percentages. Rownames are set to 'id' values that are part of
the population.
...
It should be noted, however that this function differs slightly in that
it does not distinguish between founders and non-founders in calculating
the statistic.
...
allele." We have interpreted this as meaning that genome uniqueness
...
includes living founders' alleles; it is unchanged. The genetic value
report

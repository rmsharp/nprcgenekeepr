...
An ancestor reached through both parents is returned once for each
route. The function stops with an error that names the ids involved when
`ptree` contains a cycle (an animal that is its own ancestor), which is
a data error.

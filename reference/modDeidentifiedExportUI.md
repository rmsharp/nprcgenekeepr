...
(issue \#149), this module has no `fileInput` – it reads the pedigree
the curator is already working with (D1), so there is no Upload/Validate
stage, only Configure & Preview, then Export (D2).

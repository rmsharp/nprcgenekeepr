...
rare and at risk of being lost from the population. A range of 100 to
10,000 simulations may be selected. A minimum of 1,000 simulations is
...
Genome uniqueness is an *estimate* produced by the gene-drop simulation,
so each value carries a sampling **standard error** – reported in the
`guSE` column beside it. That standard error shrinks roughly in
proportion to one over the square root of the number of iterations, so
more iterations give a more precise estimate. How many iterations are
actually *needed*, though, depends on the structure of your pedigree:
there is no single iteration count that is right for every colony.

The default is **1000 iterations**. On the pedigrees we have examined
this gives a sampling error below one percentage point in the genome
uniqueness values and – more importantly – a stable *selection order*.
The order in which animals are ranked is what actually decides which
animals are chosen for breeding, and a small `guSE` does not by itself
guarantee that order has settled. To check whether 1000 iterations is
enough for a particular pedigree, run
[`gvaConvergence()`](https://github.com/rmsharp/nprcgenekeepr/reference/gvaConvergence.md):
it recomputes the ranking across a range of iteration counts and reports
the smallest count at which the selection order stops changing. See the

...
per-module partition 8e-7 added – distinct from `R-CMD-check.yaml` and
`test-coverage.yaml`, which remain the fast, per-pull-request gate. That
...
workflow’s group array and asserts every tracked file matches exactly
one group; two groups were added for the previously orphaned files; and
the workflow’s header comment was rewritten to state the invariant
without a hardcoded file/group count, so it cannot go stale the same way

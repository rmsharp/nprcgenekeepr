...
### Phase 4 — Behavior-sensitive refactors *(§4B #11-15 + `checkRequiredCols` #1)* — **DONE (Session 56)**
...
- **✅ DONE (Session 56):** commits `17e3fa06` (checkRequiredCols RED→GREEN→REFACTOR) + `69c8d759` (5 refactors + de-exclude). **Findings during execution (all owner-gated via `AskUserQuestion`):** (1) `addSexAndAgeToGroup` (#11) is behavior-none on the *documented domain* — the `age` vapply already enforces one-row-per-id, so the planned RED concern (the `match()` error divergence) doesn't occur — so it was DOWNGRADED from a RED slice to a REFACTOR; BUT the adversarial pass (`wf_168f8dcf-1e5`) then found it diverges on **empty `ids`** (old `sapply` dropped the `sex` column → 2 cols, which CRASHED the `modBreedingGroups:438` `colnames<-` on an empty group; the `match()` form keeps the documented 3-col schema) → owner ADOPTED the robust behavior + an empty-ids contract test. (2) `create_wkbk` (#13) `if(replace)`→`if(!replace)` diverges ONLY on a non-logical `replace` while the file exists (error-message text; both still error) → owner ACCEPTED (cosmetic, out-of-contract). (3) `checkRequiredCols` (#1) confirmed a genuine RED (the planned `%in%` NA divergence held). The other 3 (`correctParentSex`/`fillGroupMembersWithSexRatio`/`setExit`) are fully behavior-none (skeptic-confirmed: 6000-iter fuzz / 146 seeded / 21 inputs). `lint_package()`=0; full suite 0/0/159 baseline; `check()` 0 errors. **#30 status: every R-file line exclusion is gone except the deliberately-kept `makeGeneticDiversityDashboard` → the line-specific-exclusion deliverable (§4) is COMPLETE; only the optional §7 residual (already 0 today) remains before #30 can close.**

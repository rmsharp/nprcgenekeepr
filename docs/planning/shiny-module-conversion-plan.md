...
*End of plan. **Phases 1–7 complete (Sessions 22–27, 29).** Next session implements **Phase 8** only — enable the shinytest2 E2E harness end-to-end: author the missing `create_app_driver`/`navigate_to_tab`/`get_html_safe` driver helpers, fix the `dataInput` namespace mismatch (`helper-shinytest2.R` hardcodes `module_id="input"`), and get the e2e tier to run green (or skip cleanly) opt-in under `NPRC_RUN_E2E=true`. This is **GitHub issue #39**. **Risk HIGH 🐉** — browser-dependent (chromote/Chromium), never run; the e2e files are unwritten theatre (20/22 AppDriver files call the 3 undefined helpers → error before asserting). See §9 Phase 8; may itself need a sub-plan (smoke → per-module → assertion-strengthening). After Phase 8: Phase 9 (declare canonical: alias `runGeneKeepR`, delete the monolith — irreversible, its own commit). Do not bundle phases.*

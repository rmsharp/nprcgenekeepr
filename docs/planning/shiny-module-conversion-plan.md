...
*End of plan. **Phases 1–5 complete (Sessions 22–26).** Next session implements **Phase 6** only (Breeding Groups parity B: seed-group pre-seeding + expose the inert `minAge`/`nIterations`/`withKinship` controls — add the monolith's seed-animal "current groups" widget [`getCurrentGroups`/`textAreaWidget`, server.r:1019-1051] replacing the hardcoded `currentGroups = list(character(0L))`, and surface the 3 controls the server reads at `modBreedingGroups.R:187-189`; see §9 Phase 6. **⚠ Breeding-sim iteration default = 10, NOT 1000** [`gpIter` value=10L, `uitpBreedingGroupFormation.R:155-161`]; the modular `1000L` fallback is a 100× drift — and unlike Phase 5 this DOES change the formed groups, so it's a real numeric change [NEWS-bullet candidate]). Do not bundle phases.*

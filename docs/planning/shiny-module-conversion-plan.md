...
| Breeding groups: downloads (group CSV, group-kinship CSV) | ✅ (S26) | server.r:1283-1297 | `downloadGroup`/`downloadGroupKin` handlers in the new "Group Detail" tab (`na=""`; member `row.names=FALSE`, kinship `row.names=TRUE`) (S26 → Phase 5) | **5 ✅** |
| Breeding groups: per-group kinship matrix + `viewGrp` selector | ✅ (S26) | server.r:1197-1280 | "Group Detail" tab: `selectInput(viewGrp)` + `bgGroupView` member table + `bgGroupKinView` (`filterKinMatrix` from the stored full `kmat`, byte-identical to `groupKin`); both views clamp to `length(breedingGroups())` (S26 → Phase 5) | **5 ✅** |
...
### Phase 5 — Breeding Groups parity A: downloads + per-group kinship + group selector · risk MEDIUM · ✅ DONE (Session 26)

> **✅ Implemented in Session 26, whole phase (no split).** All in `R/modBreedingGroups.R`: a new **"Group Detail" tab** (additive — Decision B; the existing all-groups "Groups"/"Statistics" tabs untouched) with a `viewGrp` `selectInput`, a per-group annotated member view (`addSexAndAgeToGroup` → rounded age → "Ego ID"/"Sex"/"Age in Years"), a per-group kinship matrix view, and `downloadGroup`/`downloadGroupKin` handlers. **Dragon discharged (Decision A):** the kinship view computes `filterKinMatrix(groupIds, kmat)` from the module's already-computed full `kmat` (retained in `groupResults` with a `hasUnused` flag), NOT `result$groupKin` (NULL until the Phase-6 `withKinship` control) — byte-identical to the monolith's `groupKin[[i]]` (members ⊆ candidates), and the formation path is proven `identical()` across 3 seeded scenarios. **Decision C (robust):** both views clamp `viewGrp` to `length(breedingGroups())` (the monolith clamps the member view to `input$numGp` and leaves the kinship view unclamped); selector `observe` guards on `length()>=1L` (not `req()`, which is truthy on an empty list). Tests: 5 new in `tests/testthat/test_modBreedingGroups.R` (50 total, 0/0/0). Suite 0/0, lint net-zero, `document()` zero-delta, runtime-smoked (HTTP 200). **Next: Phase 6.**
...
### Phase 6 — Breeding Groups parity B: seed-group pre-seeding + expose inert controls · risk MEDIUM · ✅ DONE (Session 27)

> **✅ Implemented in Session 27, whole phase.** All in `R/modBreedingGroups.R`: a `seedGroups` checkbox reveals one per-group `textAreaInput` (`curGrp1..N`, count driven by `nGroups`) whose IDs build a length-`numGp` `currentGroups` list fed to `groupAddAssign` in place of the hardcoded `list(character(0L))` — using `seq_len` (the monolith's `getCurrentGroups` is doubly buggy: `seq_along(input$numGp)` reads only `curGrp1`, and `vapply(...)` yields a matrix). Exposed the three previously-inert controls the server already read (L201-203) but no UI declared — `minAge` (value 1), `nIterations` (value 10L), `withKinship` — with ids matching the **server reads** (not the monolith's `gpIter`/`withKin`). Breeding-sim iteration default `1000L → 10L` (a real numeric change → NEWS bullet). **Validate-and-block** (Decision): seed IDs not in the pedigree abort formation with a notification — verified a phantom seed otherwise crashes the Phase-5 member view. Strict TDD: 7 new tests (5 RED; 2 green-at-HEAD coverage), suite 0/0, lint net-zero (31=31), `document()` zero-delta, Phase-3E HTTP 200, mutation-verified the validate-and-block guard. **Next: Phase 7.**
...
> **✅ Implemented in Session 29 (mock-wire / full parity — owner-approved via `AskUserQuestion`).** In `R/modInput.R`'s `observeEvent(input$getData)`: when `input$fileContent == "focalAnimals"`, call `getFocalAnimalPed(file$datapath, sep)` to build the pedigree from the EHR (instead of reading the focal-ID file as a pedigree); on an `nprcgenekeeprErr` (DB failure) route it to `storedErrorLst()` (cleaned NULL, early return) so the already-wired appServer Error tab surfaces `failedDatabaseConnection`. Branches on the CORRECT class `nprcgenekeeprErr` (the monolith's `is.element("nprckeepErr", …)` is a typo that never matched) and drops its dead bare-NULL branch. **2 RED tests** in `test_modInput.R` mock the LabKey seam via `testthat::local_mocked_bindings(getLkDirectRelatives = …, .package = "nprcgenekeepr")` (the real `getFocalAnimalPed` body runs; no live EHR). Suite 0/0, lint net-zero, `document()` zero-delta, Phase-3E HTTP 200. **Verification environmentally limited** (no live EHR — the live `getLkDirectRelatives`→`getDemographics` call is owner-verifiable only; stated, not skipped). **Next: Phase 8.**
...
### Phase 9 — Declare canonical: alias `runGeneKeepR`, delete monolith + orphans, update docs · risk HIGH (irreversible) 🐉 · ✅ DONE (Session 35)

> **✅ Implemented in Session 35 — the campaign is COMPLETE; issue #27 CLOSED.** `runGeneKeepR()` is now a `lifecycle::deprecate_soft()` alias launching `runModularApp(port=6013L, launch.browser=TRUE)` (zero-arg callers keep working). **Deleted `inst/application/`** (17 tracked files) as its own revertible commit (§15); `inst/www/` preserved. Removed the owner-approved orphans `getMinParentAge` (unexported), `getLogo` (exported → public-API removal), `shouldShowErrorTab` (bypassed; also dropped the dead `qcResults` build + `@seealso` refs), `modMinimalTest` (unmounted) + their tests; `document()` dropped 4 exports + 4 man pages. **NAMESPACE fallout fixed:** `getMinParentAge.R` was the sole carrier of `@import shiny` → relocated to `R/nprcgenekeepr-package.R` (else the modular UI lost `h5` — Learning #35). **Pre-flight:** re-ran the §10 inventory as a read-only multi-modal sweep + completeness critic (`wf_48a6f152-f0f`); firsthand-verified the sole `system.file` ref, `inst/www` ≠ `inst/application/www`, lifecycle dep, all-tracked. New `test_runGeneKeepR_alias.R` (deprecation + delegation + port/launch.browser forwarding) + `test_monolith_removed.R` (`system.file("application")==""`). **Verify:** non-e2e regression **2135 / 0 / 0**; runtime smoke `runGeneKeepR()` → modular **HTTP 200**; **`devtools::check()` = 0 errors / 0 warnings** (pre-existing NOTEs only). Docs/pkgdown/WORDLIST/NEWS/ROADMAP/vignette updated; README.md re-knit. Pre-existing `a2interactive.Rmd` NEW-45 vignette-table drift fixed in a separate `fix:` commit (owner-approved). Commits: `3db018d1` / `24992e0b` / `53a9e5e0` / `a1618c48`.
...
2. **Focal-animal/LabKey** (Phase 7): ✅ RESOLVED (Session 29) → **mock-wire / full parity** (owner-approved via `AskUserQuestion`); no sub-plan needed — the slice was a single `modInput.R` branch, unit-tested under `local_mocked_bindings` of the LabKey seam.
...
*End of plan. **✅ COMPLETE — all 9 phases done (Sessions 22–35).** Phases 1–7 parity (Sessions 22–27, 29); Phase 8 E2E harness mini-campaign 8a–8d (Sessions 30–34, **issue #39 CLOSED**); Phase 9 monolith retirement (Session 35, **issue #27 CLOSED**). The modular app (`runModularApp()`) is now canonical; `runGeneKeepR()` is a `lifecycle::deprecate_soft()` alias; `inst/application/` is deleted. The XARCH-1 / shiny-module conversion campaign is finished. Remaining modular-app follow-ons live in their own issues (#40 E2E assertion-strengthening + CI-stability; #30 lint debt; #35/#36 enhancements).*

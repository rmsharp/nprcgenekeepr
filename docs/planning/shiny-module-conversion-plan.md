...
*End of plan. **Phases 1–6 complete (Sessions 22–27).** Next session implements **Phase 7** only — Input parity: focal-animal / LabKey pedigree build (`getFocalAnimalPed`/`getLkDirectRelatives`; monolith server.r:86-113). **Risk HIGH 🐉** — likely needs an owner consult / sub-plan at phase start: requires `Rlabkey` + a live or mocked EHR (cannot be unit/E2E tested without a LabKey mock such as `mockery::stub` of `getLkDirectRelatives`, as `test_getFocalAnimalPed.R` already does), OR an owner-approved descope (remove the focal-animal radio option instead). See §9 Phase 7. After Phase 7: Phase 8 (E2E harness = issue #39) and Phase 9 (delete the monolith — irreversible, its own commit). Do not bundle phases.*

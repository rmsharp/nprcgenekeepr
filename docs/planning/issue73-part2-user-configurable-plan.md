...
**Status: DRAFT for owner ratification (Session 184, 2026-06-23).** This plan is a deliverable, not a license to implement (FM #18/#25). The design decisions in §3 must be ratified (owner edits this doc, or a focused `/grill-me`) before the first slice declares RED. Implementation is a separate session per slice.
...
Settle these before Slice 1 RED — a RED test cannot assert a value until D1–D4 are fixed. Each shows options and a **recommendation**.
...
- **R2 — `NULL`-default, not 2.0/210.** A configurable `default` threaded through `reportGV`/`correctUnknownParentMeanKinship` must default to `NULL` so the accessor's built-in is respected; hardcoding 2.0/210 at an upper layer breaks the no-config invariant (Mapper B).
...
- [ ] **D1** — config representation = **CSV path** (recommended) vs. inline keys.
- [ ] **D2** — CSV schema = the four `speciesGestation` columns, header required; partial-CSV allowed? (tie to D4).
- [ ] **D3** — fallbacks = two optional scalar keys (`minBreedingAgeDefault`, `gestationDefault`); confirm names.
- [ ] **D4** — **merge** (recommended) vs. replace semantics for a user CSV.
- [ ] **D5** — gestation override scope = **prefill default only** (recommended); per-animal window deferred.
- [ ] **D6** — slice order = GVA (Slice 1) → Potential Parents (Slice 2).
- [ ] **D7** — overrides ride in `shared$config` vs. a sibling `shared$speciesOverrides`; passed to modules as a reactive (per `modInputServer` precedent).
- [ ] Confirm **no Settings-tab UI** this work (owner already chose config-file only).
- [ ] Confirm **backward-compat invariant** (no config ⇒ identical to today) as a hard acceptance test in each slice.

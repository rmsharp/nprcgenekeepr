...
- **What it runs (8a–8d done):** the full opt-in tier (`filter="^(app|e2e)-"`), or at minimum the 8b smoke files; decide final breadth at 8b based on runtime/flake. **8b ran only the 3 smoke files** (`filter="^(app-loading|app-navigation|e2e-data-ready)$"`); **8c (S33) broadened it to the 18 verified 8b+8c files** (`filter="^(app-loading|app-navigation|e2e-data-ready|e2e-input|e2e-pedigree|e2e-pyramid|e2e-genetic-value|e2e-summary-statistics|e2e-breeding-groups)"` — the 5 8d files excluded until verified). **At 8d close, broaden to the full `^(app|e2e)-` tier** (all 23 files).

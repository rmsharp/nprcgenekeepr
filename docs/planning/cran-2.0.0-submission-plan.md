...
| 5b ☐ | win-builder ×3 + R-hub v2 runs + paste results + submit **(PENDING — owner-triggered. S326 readiness check found zero R/tests/DESCRIPTION drift, but the owner's actual win-builder run then surfaced a real `.Rbuildignore` gap (4 top-level files leaking into the tarball, from the S324 methodology sync) — FIXED in S327, verified via `R CMD build .` + `tar tzf`. Win-builder x3 need to be re-run against the corrected tarball; R-hub v2 was in flight against the pre-fix tree and should be re-run too. See Dragon #11 and the S327 Phase-5 status note.)** | clean platform reports; owner submits | verification/packaging | owner |

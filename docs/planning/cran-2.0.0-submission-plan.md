...
[cross-platform] win-builder x3 + R-hub v2 + cran-comments         → Phase 5a/5b  ◐ cran-comments RE-SYNC now unblocked (S322); win-builder/R-hub still PENDING, owner-run
[post-acceptance] git tag + GitHub release + dev-version bump      → Phase 6  (unchanged, after CRAN accepts)
```

Each phase remains its own session with a STOP point (FM #18/#25). **Phase 3b is next** — it is a pure documentation/version-bump session (prose + `DESCRIPTION`/`NEWS.Rmd`/README/CITATION.cff regen), REFACTOR-class like the original Phase 3, with the same TDD caveat: any `getVersion()`-dependent test must stay green, and no runtime logic changes here (the actual behavior changes it documents already shipped in their own sessions).
...
| 5a ◑ | `cran-comments.md` rewrite (§7) + cross-platform **runbook** **(DONE for June scope: S135/S136. RE-SYNC now unblocked (S322 gate landed) — update "Resubmission" section for Dragon #9's net end-state + "R CMD check results" section for S322's numbers + add the `thoughtco.com` 403 to the pre-explained-NOTEs list — this is the next session)** | cover note CRAN-facing-only; runbook commands verified; **matches the post-3b NEWS/gate numbers** | verification | 1 |
| 5b ☐ | win-builder ×3 + R-hub v2 runs + paste results + submit **(PENDING — owner-triggered; blocked behind the 5a resync, not just owner availability)** | clean platform reports; owner submits | verification/packaging | owner |

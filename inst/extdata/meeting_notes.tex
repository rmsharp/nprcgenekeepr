...
\subsection{20210806}\label{section-3}}
...
\item
  ...
  \tightlist
  \item
    Mark has not created the function to create the Tukey five number
    summary.
  \item
    Mark has created \textbf{createSimKinships} that uses
    \textbf{make\_SimPed} and \textbf{kinship} makes a list object of
    kinship matrices from simulated pedigrees of possible parents for
    animals with unknown parents.
  \item
    Mark has created \textbf{cumulateSimKinships}, which makes a list
    object of kinship matrices from simulated pedigrees of possible
...
\subsection{20210514}\label{section-4}}
...
\item
  ...
  \item
    ...
    \item
      ...
      \item
        Dams with any offspring within plus or minus 1 yr of birth but
        did not have an offspring in focal birth year. This will work
        for rhesus but needs to be made so that the exclusion period is
        variable. For example, potential baboon dams would need to be
        excluded if they had an offspring within 6 months of birth.
        Thus, the plus or minus period needs to be an optional parameter
        with a default ot 1 year and the dam exclusion period should
        default to the gestational length for the species.
      \end{itemize}
    \end{enumerate}
  \item
    Matt to work on a function to put them together into a list object
    \textbf{allSimParents} made up of lists where the internal list has
    the offspring ID (\textbf{id}), a vector of representative sires
    (\textbf{sires}), and a vector of representative dams
    (\textbf{dams}). In those cases where the sire or dam is known the
    vector will contain the known respective sire or dam ID.
  \item
    Mark to create a function (\textbf{makeSimPed()}) to use the list
    object of \textbf{allSimParents} to make the simulated pedigree
    along with the associated unit tests.
  \item
    Mark will create a function to run the simulation. It will take the
    \textbf{allSimParents} and use \textbf{makeSimPed()} to create a
    list of simulated kinship matrices. If memory becomes an issue,
    summary statistics will have to be accumulated. The simmulated
    kinship coefficients for any one animal will be normally destributed
    so this is straight forward.
  \item
    Mark will create a function to take the simulated kinship matrices
    and generate Tukey's five number summary for each.
  \end{enumerate}
\end{enumerate}

\hypertarget{section-5}{%
\subsection{20201130}\label{section-5}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Mark met with Amanda to discuss the parent exclusion project and to
  ...
  \tightlist
  \item
    The possible journals she thought of are listed below.
    ...
    \item
      Zoo Biology
    \item
      Journal of Medical Primatology - probably not good for software
    \item
      Journal of Genomics - check
    \end{enumerate}
  \end{enumerate}
\end{enumerate}

\hypertarget{section-6}{%
\subsection{20201120}\label{section-6}}
...
\item
  ...
  \item
    ...
    \item
      ...
      \tightlist
      \item
        The internal code already does this if the animal has geneotypes
        assigned. However, the user interface is minimal and does not
        provide output regarding statistical summaries of probabilities
        for those individual genes being in the current potential
        breeders.
      \end{enumerate}
    \item
      ...
      request and needs to find someone who can provide more
      explanation.
    \end{enumerate}
  \end{enumerate}
\end{enumerate}
...
\subsection{20201117}\label{section-7}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Preparation for Friday's meeting with the Genetics and Genomics
  Consortium.
\item
...
Simple metrics for the Original Version (20160816) and Current Version
1.0.3 (20200526).
...
Simple metrics for the Original Version (20160816) and Current Version
1.0.3 (20200526) and current version groups of functions.
...
\subsection{20200423}\label{section-8}}
...
\item
  ...
  \item
    ...
    \tightlist
    \item
      addGenotype.Rd: \textbackslash value
    \item
      agePyramidPlot.Rd: \textbackslash value
    \item
      allTrueNoNA.Rd: \textbackslash value
    \item
      assignAlleles.Rd: \textbackslash value
    \item
      calcFE.Rd: \textbackslash value
    \item
      calcFEFG.Rd: \textbackslash value
    \item
      calcFG.Rd: \textbackslash value
    \item
      calcRetention.Rd: \textbackslash value
    \item
      checkGenotypeFile.Rd: \textbackslash value
    \item
      convertSexCodes.Rd: \textbackslash value
    \item
      create\_wkbk.Rd: \textbackslash value
    \item
      createExampleFiles.Rd: \textbackslash arguments, {]}value
    \item
      \ldots{}
    \end{itemize}
  \item
    You have examples for unexported functions which cannot run in this
    way. Please either add nprcgenekeepr::: to the function calls in the
  ...
  \end{enumerate}
\item
  I will work toward getting these changes in as soon as possible and
  resubmit.
\item
  I have not heard from you regarding the user acceptance testing and am
  curious as whether or not you are going to go forward with that in
  this period of social distancing.
\end{enumerate}

\hypertarget{section-9}{%
\subsection{20200320}\label{section-9}}
...
\item
  ...
  \tightlist
  \item
    Correct spelling package wide - Done 2020319
  \item
    Update Development\_Plans.Rmd
    ...
    \item
      Sent out a request to Amanda and Matt for input on 20200319. Their
      input may not arrive before this submission.
    \end{itemize}
  \end{enumerate}
\end{enumerate}

\hypertarget{section-10}{%
\subsection{20200316}\label{section-10}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Matt will work on colony manager tutorial during last two weeks of
  March.
\item
...
\item
  Amanda will administer the user acceptance testing to collect feedback
  on using the application. What causes problems, what can and cannot be
  done, what is missing, what is too complex, etc.
\end{enumerate}

\hypertarget{section-11}{%
\subsection{20200203}\label{section-11}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Is the tutorial for CRAN or the Consortium users. need overview of tab
  structure and order (left to right).

  \begin{enumerate}
  \def\labelenumii{\arabic{enumii}.}
  \item
    Notes regarding feedback from Amanda on 20200204 I am willing to
    change ``reading'' to ``uploading'', but they do not mean the same
    thing. When you upload a file, there is no inference of paying
    attention to the contents. When reading a file, the contents are
    examined. Reading is the more accurate descriptor in our case.

    I am going to write a separate tutorial explicitly for the
    consortium members as typical R users would want conventional use of
    terms and descriptors. For example, Shiny apps are always described
    as Shiny apps.
  \end{enumerate}
\end{enumerate}

\hypertarget{section-12}{%
\subsection{20200106}\label{section-12}}

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\tightlist
\item
  Mark will convert nprcmanager to nprcgenekeepr before any other
  changes.
\item
  Amanda will provide notes for the Shiny application tutorial with the
  following goals.
  ...
  \tightlist
  \item
    Address the specific needs of consortium members.
  \item
    Reduce the complexity of the presentation where possible.
  \item
    Ensure there are no major components left out.
  \end{enumerate}
\item
...
\item
  ...
  \tightlist
  \item
    Mark expects that the package will be accepted on CRAN once examples
    have been provided for all exported functions. This is not a trivial
    matter as there are currently 142 exported functions.
  \item
    Mark will remove exporting of functions that do not need to be
    exported.
  \item
    Mark will use code from the unit tests for many of the examples.
  \end{enumerate}
\item
  Mark has completed only the first of the four items from the 20191120
  meeting. These will be addressed after the submission to CRAN is
  successful.
\end{enumerate}

\hypertarget{section-13}{%
\subsection{20191218}\label{section-13}}
...
\subsection{20191130}\label{section-14}}
...
\subsection{20191120}\label{section-15}}
...
\item
  ...
  Browser} tab so that full pedigree is again available. This will also
  allow a new pedigree to be added via the \textbf{Input} tab.
\end{enumerate}

\hypertarget{section-16}{%
\subsection{20191115}\label{section-16}}
...
\subsection{20191111}\label{section-17}}
...
\subsection{20190916}\label{section-19}}
...
\toprule()
id & sire & sire\_from\_center & dam & dam\_from\_center & birth \\
\midrule()
\endhead
...
\subsection{20190826}\label{section-20}}
...
\subsection{20190810}\label{section-21}}
...
\subsection{20190715}\label{section-22}}
...
\item
  ...
  of kinship relationships within breeding groups
...
\subsection{20190603}\label{section-23}}
...
\subsection{20190429}\label{section-24}}
...
\subsection{20190408}\label{section-25}}
...
\subsection{20190311}\label{section-26}}
...
\subsection{20190225}\label{section-27}}
...
\item
  ...
  \item
    ...
    mutually exclusive within a run.
  \end{enumerate}
\item
...
\subsection{20190114}\label{section-28}}
...
\subsection{20181210}\label{section-29}}
...
overlay among the various subgroups. This is complected and hard to
...
\subsection{20181105}\label{section-30}}
...
\item
  ...
  have trapped the error with a tryCatch function and am sending a
...
\subsection{20181022}\label{section-31}}
...
\subsection{20180917}\label{section-32}}
...
\subsection{20180820}\label{section-33}}
...
\paragraph{20180429}\label{section-34}}

Test coverage is 86.32\%.

\hypertarget{section-35}{%
\paragraph{20171004}\label{section-35}}

Unit test coverage is 84.79\%.

\hypertarget{section-36}{%
\paragraph{20170921}\label{section-36}}

Unit test coverage is about 15\%.

\hypertarget{section-37}{%
\paragraph{20170919}\label{section-37}}
...
\paragraph{20170917}\label{section-38}}
...
\paragraph{20170916}\label{section-39}}
...
\paragraph{20170915}\label{section-40}}

Added a genetic uniqueness plot to the Summary Statistics tab.

\hypertarget{section-41}{%
\paragraph{20170911}\label{section-41}}

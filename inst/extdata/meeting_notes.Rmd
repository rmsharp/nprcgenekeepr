...
1.   Mark met with Amanda to discuss the parent exclusion project and to 
     discuss writing of a paper about mprcgenekeepr. 
...
1.   Mark and Matt met with the Genetics and Genomics Consortium to present a
     ...
     1.  Parent exclusion software
         ...
         1.  May integrate into `mprcgenekeepr`
...
threshold of 10 minutes. This can for example be achieved by:
    ...
    1.  You have examples for unexported functions which cannot run in this way.
        Please either add mprcgenekeepr::: to the function calls in the 
...
## 20200106 
1.  Mark will convert nprcmanager to mprcgenekeepr before any other changes.
...
if (Sys.Date() > "2020-08-01") {
  path <- file.path("/Users", "rmsharp", "Documents", "Development", "R",
                    "r_workspace")
} else {
  path <- file.path("/Users", "rmsharp", "Documents", "Development", "R",
                    "r_workspace", "library")
...
originalFiles <- file.path(
  "/Users", "rmsharp", "Documents", "Projects", "Active_Projects",
  "mprcgenekeepr_project", "20160816_GeneticManagementTools"
)
functions <- file.path(path, "mprcgenekeepr", "R")
tests <- file.path(path, "mprcgenekeepr", "tests", "testthat")
ui <- file.path(path, "mprcgenekeepr", "inst", "application")
...
sectionCodeChanges <- rbind(codeChanges, codeCounts[-1L, ],
...
for (i in 1L:4L) {
 unique <- sample(LETTERS, 15L, replace = FALSE)
 group <- stri_c("Overlap_Group_", i)
 overlapping_groups[[group]] <- data.frame(unique_set_1 = unique[1L:5L],
                                           unique_set_2 = unique[6L:10L],
                                           unique_set_3 = unique[11L:15L],
                                           stringsAsFactors = FALSE)
}
overlapping_str <- vapply(overlapping_groups, function(x) {
  stri_c("Unique Set 1: ", stri_c(x[[1L]], collapse = ","), "\n",
         "Unique Set 2: ", stri_c(x[[2L]], collapse = ","), "\n",
         "Unique Set 3: ", stri_c(x[[3L]], collapse = ","), "\n")
}, character(1L))
cat(overlapping_str)
```
